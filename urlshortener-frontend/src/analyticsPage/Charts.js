import { PieChart, Pie, Cell,Tooltip, BarChart, Bar, XAxis, YAxis,  ResponsiveContainer, CartesianGrid, AreaChart, Area } from "recharts";
import "./Charts.css";
...
export const DeviceBarChart = ({ data }) => {
    ...
    const COLORS = ["#5AC8FA", "#34C759", "#FF9500", "#AF52DE", "#FF3B30"]; // Apple colors
  
    return (
      <BarChart width={350} height={260} data={barData}>
        <XAxis dataKey="name" />
        <YAxis />
        <Tooltip />
        <Bar
  ...
  export const EngagementAreaChart = ({ data }) => {
    ...
    return (
      <ResponsiveContainer width="100%" height={260}>
        <AreaChart data={clickData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
          <defs>
          ...
          <YAxis />
          <Tooltip />
          <Area type="monotone" dataKey="clicks" stroke="#34C759" fill="url(#colorClicks)" strokeWidth={3} />
        </AreaChart>
      </ResponsiveContainer>
    );
  };
